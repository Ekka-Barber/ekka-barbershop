...
export abstract class BaseCalculator {
   ...
   */
  getFromCache(_params: CalculationParams): CalculatorResult | null {
   ...
   * Can be overridden by subclasses for specific caching strategies
   *
   * @param _key Cache key to store result under
   * @param _result Calculation result to cache
   */
  saveToCache(_key: string, _result: CalculatorResult): void {
   ...
   */
  protected handleCalculationError(
    error: unknown,
