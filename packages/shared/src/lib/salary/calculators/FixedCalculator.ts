...
export class FixedCalculator extends BaseCalculator {
  async calculate(params: CalculationParams): Promise<CalculatorResult> {
    try {
      ...
      logger.info(
        `Calculating fixed salary for ${params.employee.name} with plan ID ${params.plan.id}`,
        {
      ...
      if (config.blocks && Array.isArray(config.blocks)) {
        ...
        for (const block of config.blocks) {
          if (
            block.type === 'fixed_amount' &&
            block.config &&
            block.config.amount
          ) {
            baseSalary = Number(block.config.amount);
            logger.info(`Found fixed_amount in block: ${baseSalary}`);
            break;
          }

          // Also check for basic_salary block (legacy format)
          if (
            (block.type === 'basic_salary' || block.type === 'fixed') &&
            block.config
          ) {
            ...
            if (blockAmount > 0) {
              ...
              logger.info(`Found fixed salary in legacy block: ${baseSalary}`);
              break;
            }
          }
        }
      ...
      if (baseSalary === 0) {
        ...
        logger.info(
          `Direct config properties found base salary: ${baseSalary}`
        );
      }
      ...
      if (baseSalary === 0) {
        logger.warn(
          `Could not find valid fixed salary amount for ${params.employee.name}. Check salary plan configuration.`
        );
      }
