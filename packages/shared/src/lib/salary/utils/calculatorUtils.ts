 ...
 */
export function extractBaseSalary(config: Record<string, unknown>): number {
  let baseSalary = 0;
  ...
  if (config.blocks && Array.isArray(config.blocks)) {
    ...
    baseSalary = Number(
      config.base_salary || config.base_amount || config.amount || 0
    );
 ...
 */
export function extractCommissionParams(config: Record<string, unknown>): {
  ...
  if (config.blocks && Array.isArray(config.blocks)) {
    ...
    for (const block of config.blocks) {
      ...
      if (block.type === 'commission' && block.config) {
        commissionRate = Number(
          block.config.rate || block.config.commission_rate || 0
        );
        threshold = Number(block.config.threshold || 0);
        logger.info('Found commission block:', {
          rate: commissionRate,
          threshold: threshold,
          currency: 'SAR',
        });
    ...
    logger.info('Extracted commission params from flat config:', {
      rate: commissionRate,
      threshold: threshold,
    });
  }
 ...
 */
export function extractTargetBonus(config: Record<string, unknown>): number {
  let targetBonus = 0;
  ...
  if (config.blocks && Array.isArray(config.blocks)) {
    ...
    for (const block of config.blocks) {
      ...
      if (block.type === 'target_bonus' && block.config) {
        targetBonus = Number(
          block.config.amount || block.config.bonus_amount || 0
        );
