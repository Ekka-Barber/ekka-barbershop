...
export const usePDFFetch = (
  ...
  const fetchPDFFiles = async (): Promise<PDFFile[]> => {
    ...
    if (data && data.length > 0) {
      ...
      const marketingFiles = data as any[];
      // Get public URLs for all files
      const filesWithUrls = await Promise.all(marketingFiles.map(async (file) => {
        if (!file) return null;
        ...
        const enhancedFile: PDFFile = {
          ...
          end_date: file.end_date || undefined
        };
      ...
      const validFiles = filesWithUrls.filter((file) => file !== null) as PDFFile[];
